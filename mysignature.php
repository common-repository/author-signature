...
Copyright 2020  Zakir Sajib  (email : zakirsajib@gmail.com)
...
if ( ! class_exists( 'MySignature' ) ) {
	class MySignature {
		public function __construct() {
			...
			add_filter( 'the_content', array( $this, 'post_signature' ), -1 );
		}
		...
		public function post_signature( $content ) {
			...
			$sig = '<p class="my-signature">&mdash; ' . esc_html($display_name) . '</p>';
	
			$user_signature = esc_url( get_the_author_meta( 'user_signature', $author_id ) );
					...
					if ( class_exists( 'woocommerce' ) ) {
						...
						&& ( ! is_account_page() ) ) {
							...
							if ( ! empty( $user_signature ) ):
								...
								$content .= '<div class="woo-activated signature-' . $alignment . '">';
								$content .= "Signature image is missing";
								$content .= '</div>';
							endif;
						}
					} else {
...
if ( class_exists( 'MySignature' ) ) {
	register_activation_hook( __FILE__, array( 'MySignature', 'activate_MySignature' ) );
	register_deactivation_hook( __FILE__, array( 'MySignature', 'deactivate_MySignature' ) );
